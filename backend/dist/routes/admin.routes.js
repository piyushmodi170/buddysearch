...
router.get('/stats', adminAuth, async (req, res) => {
    try {
        ...
        const [totalUsers, verifiedUsers, bannedUsers, pendingVerifications, newToday, onlineNow, clients, buddies, both, totalRequests, openRequests, totalOffers, totalChats, totalMessages, totalReviews, revenueAll, revenueMonth, successfulPayments, pendingPayments, recentSignups] = await Promise.all([
            prisma.user.count(),
...
router.put('/plans/:id', adminAuth, async (req, res) => {
    try {
        ...
        if (displayName !== undefined)
            patch.displayName = String(displayName);
        if (tagline !== undefined)
            patch.tagline = String(tagline);
        if (price !== undefined)
            patch.price = Number(price);
        if (originalPrice !== undefined)
            patch.originalPrice = Number(originalPrice);
        if (discount !== undefined)
            patch.discount = Number(discount);
        if (durationMonths !== undefined)
            patch.durationMonths = Number(durationMonths);
        if (postLimit !== undefined)
            patch.postLimit = Number(postLimit);
        if (features !== undefined)
            patch.features = features;
        if (isPopular !== undefined)
            patch.isPopular = Boolean(isPopular);
